  ...
  CLaserOdometry2D()
{
  ...
  pn.param<std::string>("laser_frame_alias",laser_frame_alias,"/laser");
  ...
  while ( ros::ok() && !tf_listener.frameExists(base_frame_id) )
  {
    //ROS_WARN_STREAM("Waiting for /base_frame_id to be published.");
    ros::Duration(0.01).sleep();
  }
  //ROS_WARN_STREAM("/base_frame_id published.");

  first_laser_scan   = true;

  while ( ros::ok() && first_laser_scan == true ) //get 'last_scan.header.frame_id' before tf lookup
  {
    //ROS_WARN_STREAM("Waiting for first laser scan.");
    ros::spinOnce(); 
    ros::Duration(0.01).sleep();
  }
  ...
  first_laser_scan = true; //Because added the while loop right above, need to reset the init robot pose
...
/*  try
  {
    tf_listener.lookupTransform(base_frame_id, laser_frame_alias, ros::Time(0), transform);
...
*/
  if (tf_listener.frameExists(base_frame_id) && tf_listener.frameExists(laser_frame_alias))
  {
    tf_listener.lookupTransform(base_frame_id, laser_frame_alias, ros::Time(0), transform);
    retrieved = true;
  }
